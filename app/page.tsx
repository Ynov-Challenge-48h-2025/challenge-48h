...
import { updateDistrictsFromZoneData } from "./services/dataService";
...
export default function Home() {
    ...
    useEffect(() => {
        if (!mapReady || !map || Object.keys(arrondissements).length === 0)
            return;
    ...
    useEffect(() => {
        const intervalId = startDataSimulation(
            (newData: Record<string, ZoneData>) => {
                setZoneData(newData);

                Object.values(newData).forEach((data: ZoneData) => {
                    setDistricts((currentDistricts) =>
                        updateDistrictsFromZoneData(data, currentDistricts)
                    );
                });
            },
            1000
