...
export default function Home() {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      try {
        const { arrondissements: arrondData, districts: districtsData } = await fetchGeoData(map);
  ...
  useEffect(() => {
    const intervalId = startDataSimulation((newData: Record<string, ZoneData>) => {
      ...
      Object.values(newData).forEach((data: ZoneData) => {
        setDistricts(currentDistricts => updateDistrictsFromZoneData(data, currentDistricts));
      });
    }, 1000); 
